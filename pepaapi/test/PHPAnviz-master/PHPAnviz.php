 ...
 */
 ...
 class PHPAnviz {
    ...
    function __construct($id, $port, $configFilePath = '') {
        ...
        socket_connect($this->client, '192.168.1.16', 5010);
    }
...
//        echo "Envio: ".var_Export($req,true);

//echo "Comando ".$commands[0]."  \n";

        socket_write($this->client, hex2bin($commands[0]));
//        sleep(2);

        $rescmd = socket_read($this->client, 1000);
        if ($type==0)
        $res = bin2hex($rescmd);
        else
        $res = array(bin2hex($rescmd));

//        echo "Recibo: ".var_Export($res,true);

        $parsed=$this->parseResponse($res, $type);

 //       echo "Parsed: ".var_Export($parsed,true);
...
echo "grablo lang: $language \n";
     ...
     * @access public
     */
    public function setInfo2($fingerprint_comparison_precision=0xFF,$fixed_wiegand_head_code=0xFF,$wiegand_option=0xFF,$work_code_permission =0xFF, $real_time_mode_setting=0xFF, $fp_auto_update =0xFF, $relay_mode =0xFF, $lock_delay =0xFF, $memory_full_alarm =0xFF, $repeat_attendance_delay =0xFF, $door_sensor_delay =0xFF, $scheduled_bell_delay =0xFF, $reserved =0xFF) {



        $data = sprintf("%02x%02x%02x%02x%02x%02x%02x%02x%06x%02x%02x%02x%02x", $fingerprint_comparison_precision, $fixed_wiegand_head_code, $wiegand_option, $work_code_permission, $real_time_mode_setting, $fp_auto_update, $relay_mode, $lock_delay, $memory_full_alarm, $repeat_attendance_delay, $door_sensor_delay, $scheduled_bell_delay, $reserved);
echo "envio comando: 0x33 $data \n";
        $commands = $this->buildRequest(0x33, $data);

        $res = $this->request($commands);

        if ($res['ret'] == PHPAnviz::ACK_SUCCESS && $res['ack'] == 0xB3) {
            return true;
        }
    ...
    function uploadStaffInfo($users) {
        ...
        foreach ($users as $user) {
            $name = unpack('H*', $user['name']);

            $name = $this->fixName($name[1]);

            $employee = [
                'user_id' => sprintf('%010x', $user['user_id']),
                'pwd' => sprintf('%06x', $user['pwd']),
                'card_id' => sprintf('%08x', $user['card_id']), //$user[2]);
...
/**
     * Upload FP Template from T&A device
     * @param int $user
     * @param hex $backup_code
     * @return FP Template|false
     * @access public
     */
    public function uploadFPTemplate($user, $backup_code, $template) {
        $data = sprintf('%010x%02x%s', $user, $backup_code,$template);
        $commands = $this->buildRequest(0x45, $data);

        $res = $this->request($commands);
        if ($res['ret'] == PHPAnviz::ACK_SUCCESS && $res['ack'] == 0xC5) {
            return true;
        }
    ...
    /**
     * Force T&A device reboot
     * @return boolean
     * @access public
     */
    public function reboot() {
        //0x4F inicializa el sistema
        $data = sprintf("%02x", 0x60);
        $commands = $this->buildRequest(0x2E,$data);

        $res = $this->request($commands);
        if ($res['ret'] == PHPAnviz::ACK_SUCCESS && $res['ack'] == 0xAE) {
            return true;
        }
...
/**
     * Force T&A device output realtime
     * @return boolean
     * @access public
     */
    public function sendTARealTime() {
        $commands = $this->buildRequest(0x5F);

        $res = $this->request($commands);

        if ($res['ret'] == PHPAnviz::ACK_SUCCESS && $res['ack'] == 0xDF) {
            return true;
        }
    ...
    public function test($command,$data) {
        $data = sprintf("%02x",$data );
        $commands = $this->buildRequest($command,$data);
        $res = $this->request($commands);
     ...
     * Download FP Template from T&A device
     * @param int $user
     * @param hex $backup_code (Figerprint id)
     * @return FP Template|false
     * @access public
     */
    public function enrollFP($user, $backup_code = 0x01, $enroll_time = 0x01) {
        $data = sprintf('%010x%02x%02x', $user, $backup_code,$enroll_time);
        $commands = $this->buildRequest(0x5C, $data);
        $res = $this->request($commands);

        if ($res['ret'] == PHPAnviz::ACK_SUCCESS && $res['ack'] == 0xDC) {
            return true;
        } else {
            switch ($res['ret']) {
                case PHPAnviz::ACK_FAIL:
                    echo "capture fallo";
                    break;
                case PHPAnviz::ACK_TIME_OUT:
                    echo "capture timeout";
                    break;
                case PHPAnviz::ACK_USER_OCCUPIED:
                    echo "user already exists";
                    break;
                case PHPAnviz::ACK_FINGER_OCCUPIED:
                    echo "fingerprint already exists";
                    break;
                default:
                    echo "Resultado: ".$res['ret'];
                    break;
            }
            echo "\n";
        }
