...
//            return false;
    ...
    public function leeCredencial($data) {
        /**
         * "" = permitido, "N" = invÃ¡lido, "R" = rechazado, "H" = tipo habilitacion incorrecto, "S" = no existe Componente para credencial, 
         * "V" = no estÃ¡ vigente, "T" = tiempo de ingreso en temporal, "C" = cantidad ingresos en hab temporal, 
        ...
        //BUSCA SI EL Componente e IO EXISTE
        ...
        if (!$tema_localizado) {
            ...
            return response(['rs485' => array()], Response::HTTP_NOT_FOUND);
        }
        ...
        if ($ind_movimiento == "L") {
            ...
            Broadcast::driver('fast-web-socket')->broadcast(["input"], 'info',  $context);
            return response(['rs485' => $res_ok], Response::HTTP_OK);
        }
        ...
        if ($ind_rechazo !== "") {
            ...
            $context=array(
                'msgtext'=>"Tarjeta: $tarjeta $des_credencial, Persona: $nro_documento $ape_persona $nom_persona, OrganizaciÃ³n: $nom_ou_hab, Tema: $nom_tema, Mov: $des_movimiento, Tipo Hab: $tipo_habilitacion, Sector: $muestro_sector ($ind_rechazo)",
                'cod_credencial' => $cod_credencial, 
                'cod_tema' => $cod_tema_origen,
                'ind_rechazo' => $ind_rechazo
            );

            Broadcast::driver('fast-web-socket')->broadcast(["movcred"], 'error',  $context);

            return response(['rs485' => $res_error], Response::HTTP_OK);
        } else {
            ...
            if ($ind_permanencia && $ind_movimiento == "I") {
            ...
            $context = array('cod_credencial' => $cod_credencial, 'cod_tema' => $cod_tema_origen, 'ind_rechazo' => $ind_rechazo);

            $context['msgtext']="Tarjeta: $tarjeta $des_credencial, Persona: $nro_documento $ape_persona $nom_persona, OrganizaciÃ³n: $nom_ou_hab, Tema: $nom_tema, Mov: $des_movimiento, Tipo Hab: $tipo_habilitacion, Sector: $muestro_sector";
            Broadcast::driver('fast-web-socket')->broadcast(["movcred"], 'info',  $context);
    ...
    public function leeCredencial2($data) {
        /**
